...
from app.models import ColumnId
...
def test_done_tasks_are_auto_archived_after_seven_days(client, auth_headers):
    old_done_task = store.tasks["task-4"]
    old_done_task.completed_at = date.today() - timedelta(days=8)
    ...
    assert store.tasks["task-4"].archived is True


def test_done_tasks_on_cutoff_are_auto_archived(client, auth_headers):
    cutoff_task = store.tasks["task-4"]
    cutoff_task.completed_at = date.today() - timedelta(days=7)
    cutoff_task.column = ColumnId.DONE
