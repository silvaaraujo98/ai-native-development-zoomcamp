...
transaction seam. PostgreSQL claims use row locking with SKIP LOCKED; SQLite
retains its serialized writer transaction for local development and tests.
"""
...
def claim_one(agent_id: str, worker_id: str | None) -> dict[str, Any] | None:
    with immediate_transaction() as db:
        ...
        task = db.scalar(
            ...
            .with_for_update(skip_locked=not DATABASE_URL.startswith("sqlite"))
            .limit(1)
        )
        if task is None:
            return None
